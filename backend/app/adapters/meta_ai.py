"""Meta AI Blog adapter — HTML index scraping (no public RSS)."""
...
from app.adapters.article_html import extract_meta_ai_article, fetch_html
from app.adapters.base import BaseSourceAdapter
from app.adapters.html_index import extract_meta_ai_candidates
...
class MetaAIBlogAdapter(BaseSourceAdapter):
    source_name = "Meta AI Blog"
    adapter_key = "meta_ai_blog"
    ...
    async def fetch_index(self) -> list[dict[str, Any]]:
        ...
        try:
            async with ingestion_http_client(self.source_config) as client:
                ...
                raw_entries = extract_meta_ai_candidates(resp.text, base_url=base)
    ...
    async def fetch_article(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        extracted = extract_meta_ai_article(html, final_url)
